...
{
    // 计算全局线程索引，并行度改为 natoms * neigh_num
    int global_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (global_idx >= natoms * neigh_num) return;

    int atom_id = global_idx / neigh_num;
    ...
    if (atom_id < natoms) {
        ...
        for (int n = 0; n < n_max; ++n) {
            ...
            for (int k = 0; k < n_base; ++k) {
                ...
                // }
                if (n == 0) {
                    atomicAdd(&dfeat_c2[dfeat_c_start_idx + t2 * n_base + k], fn12[k]); //[batch, n_atom, J_Ntypes, N_base]
                    dfeat_2b_noc[d2bnoc_idx + k * 4] += fnp12[k];
                }
            }
            atomicAdd(&feat_2b[feat_start_idx + n], gn12);
            // if (atom_id==23){
